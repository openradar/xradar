...
* DOC: ``nexrad_read_chunks`` notebook now falls back to the open-radar-data fixture when the live S3 listing yields no complete sweep. The guard only caught ``ValueError``, but a listing holding just the start (S) chunk parses fine and returns a ``DataTree`` with no sweep groups, so the fallback was skipped and the notebook failed on ``dtree["sweep_0"]`` ({pull}`377`) by [@aladinor](https://github.com/aladinor)
