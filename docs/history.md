...
* ADD: NEXRAD Level 3 (NIDS) radial-product reader (``NexradLevel3BackendEntrypoint``, ``open_nexradlevel3_datatree``). Supports digital radial (packet 16), run-length encoded (packet AF1F) and generic (packet 28, XDR) products — super-res reflectivity/velocity/spectrum width, storm-relative velocity, dual-pol ZDR/CC/KDP, hydrometeor classification and precipitation accumulations/rates — with values decoded to physical units (m/s, mm) and range-folded bins exposed as a separate mask. Adds ``SRMV``/``ACCUM``/``HCLASS`` to the canonical moment names in ``xradar.model`` by [@mgrover1](https://github.com/mgrover1)
...
* MNT: Transform ipynb notebooks to myst markdown notebooks ({issue}`342`, {pull}`348`) by [@kmuehlbauer](https://github.com/kmuehlbauer)
