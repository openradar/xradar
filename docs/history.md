...
* ENH: Add xarray-native ``open_datatree`` with ``engine=`` parameter for all 11 backends, enabling ``xd.open_datatree(file, engine="odim")`` and ``xr.open_datatree(file, engine="odim")`` ({issue}`329`, {pull}`335`) by [@aladinor](https://github.com/aladinor)
