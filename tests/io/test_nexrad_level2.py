...
def test_dynamic_sweeps_initialization(nexradlevel2_files):
    """Test dynamic initialization of sweeps based on input type."""

    def initialize_sweeps(sweep, filename_or_obj):
        sweeps = []
    ...
    # Run initialization logic with different sweep input types
    test_cases = [
        ("sweep_0", nexradlevel2_files, ["sweep_0"]),
        (0, nexradlevel2_files, ["sweep_0"]),
        ([0, 1, 2], nexradlevel2_files, ["sweep_0", "sweep_1", "sweep_2"]),
        (
            None,
            nexradlevel2_files,
            [f"sweep_{i}" for i in range(11)],
        ),  # default behavior
    ]

    for sweep_input, filename, expected_sweeps in test_cases:
        sweeps = initialize_sweeps(sweep_input, filename)
        assert sweeps == expected_sweeps, f"Failed for sweep input: {sweep_input}"


# @pytest.mark.parametrize(
#     "sweeps_input, expected_sweeps",
#     [
#         ("/sweep_0", ["sweep_0"]),
#         (0, ["sweep_0"]),
#         ([0, 1, 2], ["sweep_0", "sweep_1", "sweep_2"]),
#         (["/sweep_0", "/sweep_1"], ["sweep_0", "sweep_1"]),
#         (None, [f"sweep_{i}" for i in range(16)]),
#     ],
# )
# def test_open_nexradlevel2_datatree_sweeps_initialization(
#     nexradlevel2_file, sweeps_input, expected_sweeps
# ):
#     """Test that `open_nexradlevel2_datatree` correctly initializes sweeps."""
#
#     kwargs = {
#         "first_dim": "auto",
#         "reindex_angle": False,
...
#     }
#
#     # Pass `sweeps_input` directly instead of through kwargs
#     dtree = open_nexradlevel2_datatree(nexradlevel2_file, sweep=sweeps_input, **kwargs)
#
#     actual_sweeps = list(dtree.match("sweep_*").keys())
#     assert actual_sweeps == expected_sweeps, f"Unexpected sweeps for input: {sweeps_input}"
